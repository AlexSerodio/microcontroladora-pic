
int AD;   // pino analogico digital de 0 a 1023
char TXT[7];
unsigned short temperatura;
float step = (5.0 / 1024.0);

void pula_linha() {
     UART1_Write(13);
     UART1_Write(10);
}

void main() {
     
     UART1_Init(19200);
     ADCON1 = 0B00001110; // configura se as portas são digitais (0) ou analogicas (1)
     TRISB = 0B00001111;  // saida RB7 a RB4 | entrada RB3 a RB0
     PORTB = 0B00000000;  // portas ligadas ou desligadas



     while(1) {
          AD = ADC_Read(0);
          temperatura = (float) AD * step * 100;
          
          if(temperatura >= 0 && temperatura <= 24) {
            // d4
            PORTB = 0B10000000;
          } else if(temperatura >= 25 && temperatura <= 33) {
            // d3, d4
            PORTB = 0B11000000;
          } else if(temperatura >= 34 && temperatura <= 43) {
            // d2, d3, d4
            PORTB = 0B11100000;
          } else if(temperatura >= 44 && temperatura <= 89) {
            // d1, d2, d3, d4
            PORTB = 0B11110000;
          }
          
          inttostr(temperatura, TXT);
          UART1_Write_Text(TXT);
          pula_linha();
     }

}
